...
class WeatherMap:
    ...
    def smooth_dslev(self, dslev):
        dslev["theta_w"] = mcalc.smooth_gaussian(dslev["theta_w"], 5)
        dslev["T"] = mcalc.smooth_gaussian(dslev["T"], 6)
        dslev["w"] = mcalc.smooth_gaussian(dslev["w"], 6)
        dslev["vp"] = mcalc.smooth_gaussian(dslev["vp"], 5)
        dslev["vo"] = mcalc.smooth_gaussian(dslev["vo"], 5)
        dslev["T-Tw"] = mcalc.smooth_gaussian(dslev["vo"], 5)

    def smooth_dssfc(self, dssfc):
        dssfc["pmsl"] = mcalc.smooth_gaussian(dssfc["pmsl"], 6)
        ...
        if ax is None:
            ...
            ax = fig.add_subplot(
                ...
                projection=ccrs.LambertConformal(
                    central_longitude=140, central_latitude=30
                ),
            )
        ax.coastlines(resolution="50m")
        gl = ax.gridlines(
            draw_labels=True,
            xlocs=plt.MultipleLocator(10),
    ...
    def plot_surface_ps_wind(self, ds, ax=None, fig=None, map="EastAsia"):
        ...
        cs = ax.contour(
            ...
            levels=self.levels["SFC_FCT"]["level1"],
            colors="k",
        ...
        cs2 = ax.contour(
            ...
            ds["pmsl"] * 1e-2,
            transform=ccrs.PlateCarree(),
            levels=self.levels["SFC_FCT"]["level2"],
            colors="k",
            linewidths=self.boldlinewidth,
        )
    ...
    def plot_850hPa_map(
        ...
        cs = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["850hPa"]["level1"],
            colors="k",
            linewidths=self.linewidth,
        )
        # 太線
        cs2 = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["850hPa"]["level2"],
            colors="k",
            linewidths=self.boldlinewidth,
        )
        ...
        cs3 = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["T"].sel(level=lev) - 273.15,
            transform=ccrs.PlateCarree(),
            levels=self.Tlevels[season],
            colors="k",
            linestyles="dashed",
            linewidths=self.linewidth,
        )
        ...
        ax.barbs(
            ds["lon"],
            ds["lat"],
            ds["u"].sel(level=lev).values / 0.51,
            ds["v"].sel(level=lev).values / 0.51,
            length=6,
            regrid_shape=12,
            transform=ccrs.PlateCarree(),
    ...
    def plot_700hPa_map(self, ds, lev=700, fig=None, ax=None, map="EastAsia"):
        ...
        cs = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["700hPa"]["level1"],
            colors="k",
            linewidths=self.linewidth,
        )
        cs2 = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["700hPa"]["level2"],
            colors="k",
            linewidths=self.boldlinewidth,
        )
        ...
        cs3 = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["T"].sel(level=lev).values - 273.15,
            transform=ccrs.PlateCarree(),
            levels=self.Tlevels["cool"],
            colors="k",
            linestyles="dashed",
            linewidths=self.boldlinewidth,
        )
    ...
    def plot_500hPa_map(
        self, ds, season="warm", lev=500, fig=None, ax=None, map="EastAsia"
    ):
        fig, ax = self._generate_figure(fig=fig, ax=ax, map=map)
        # 等高度線
        cs = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["500hPa"]["level1"],
            colors="k",
            linewidths=self.linewidth,
        )
        cs2 = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["500hPa"]["level2"],
            colors="k",
            linewidths=self.boldlinewidth,
        )
    ...
    def plot_300hPa_map(
        self, ds, season="warm", lev=300, ax=None, fig=None, map="EastAsia"
    ):
        fig, ax = self._generate_figure(fig=fig, ax=ax, map=map)
        # 等高度線
        cs = ax.contour(
            ...
            ds["hgt"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=self.levels["300hPa"]["level1"],
            colors="k",
            linewidths=self.boldlinewidth,
        )
        ax.clabel(cs, cs.levels[::2])
        ...
        cs4 = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["wind"].sel(level=lev),
            transform=ccrs.PlateCarree(),
            levels=np.arange(0, 100.1, 20),
            colors="k",
            linestyles="dashed",
            linewidths=self.linewidth,
        )
    ...
    def _detect_peaks_v(
        self, dsinput, item, filter_size=3, order=0.3, factor=1.0e0, map="EastAsia"
    ):
        ...
        local_max = maximum_filter(
            val, footprint=np.ones((filter_size, filter_size)), mode="constant"
        )
        ...
        peak_values_max = np.ma.array(
            detected_peaks_max,
            mask=~(detected_peaks_max >= detected_peaks_max.max() * order),
        )
        peaks_index_max = np.where((peak_values_max.mask != True))
        peak_values_min = np.ma.array(
            detected_peaks_min,
            mask=~(detected_peaks_min <= detected_peaks_min.min() * order),
        )
    ...
    def _plot_peak_v(self, ax, df_peak):
        ...
        for lon, lat, label in zip(
            df_peak_sort.loc[peak_list2, "lon"],
            df_peak_sort.loc[peak_list2, "lat"],
            df_peak_sort.loc[peak_list2, "label"],
        ):
    ...
    def _detect_peaks_precip(self, dsinput, filter_size=3, order=0.3, map="EastAsia"):
        ...
        precip = ds.values
        local_max = maximum_filter(
            precip, footprint=np.ones((filter_size, filter_size)), mode="constant"
        )
        detected_peaks = np.ma.array(precip, mask=~(precip == local_max))
        # 小さいピーク値を排除（最大ピーク値のorder倍のピークは排除）
        peak_values = np.ma.array(
    ...
    def _plot_peak(self, ax, df_peak):
        ...
        for lon, lat, label in zip(
            ...
            ax.scatter(
                lon, lat, color="black", transform=ccrs.PlateCarree(), marker=".", s=1
            )
            ax.text(
                lon,
                lat,
    ...
    def plot_surface_ps_wind_precip(
        self, ds, cmap=jmacmap, alpha=0.8, ax=None, fig=None, map="EastAsia"
    ):
        precip = ds["precip"]
        # 極大値を見つける
    ...
    def plot_850hPa_T_wind_700hPa_omega(
        self, ds, cmap="none", ax=None, fig=None, map="EastAsia"
    ):
        fig, ax = self._generate_figure(fig=fig, ax=ax, map=map)
        # 850hPa気温
        cs = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["T"].sel(level=850).values - 273.15,
            transform=ccrs.PlateCarree(),
            colors="k",
        ...
        # 鉛直p速度の極大値
        df_peak = self._detect_peaks_v(ds, "vp", filter_size=3, order=0.3, factor=1.0)
        self._plot_peak(ax, df_peak)
        ...
        ax.barbs(
            ds["lon"],
            ds["lat"],
            ds["u"].sel(level=850).values / 0.51,
            ds["v"].sel(level=850).values / 0.51,
            length=6,
            regrid_shape=12,
            transform=ccrs.PlateCarree(),
            linewidth=self.barbwidth,
    ...
    def plot_500hPa_T_700hPa_dew_point_depression(
        self, ds, cmap="none", ax=None, fig=None, map="EastAsia"
    ):
        fig, ax = self._generate_figure(fig=fig, ax=ax, map=map)
        # 500hPa等温線
        ...
        cs3 = ax.contour(
            ds["lon"],
            ds["lat"],
            ds["T"].sel(level=500).values - 273.15,
            transform=ccrs.PlateCarree(),
            colors="k",
            linewidths=self.boldlinewidth,
            linestyles="solid",
    ...
    def plot_850hPa_wind_equ_potential_temperature(
        self, ds, ax=None, fig=None, map="EastAsia"
    ):
        fig, ax = self._generate_figure(fig=fig, ax=ax, map=map)
        # 850hPa風矢羽根プロット
        ax.barbs(
